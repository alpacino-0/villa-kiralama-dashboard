...
import { Villa, VillaStatus, type VillaUpdate } from "@/types/villa"
...
interface VillaFormProps {
  ...
  villa?: Villa; // Mevcut villa verileri (düzenleme durumunda)
}

export function VillaForm({ regions, villa }: VillaFormProps) {
  ...
  // Form tanımlaması - varsayılan değerleri villanın mevcut değerlerinden al
  const form = useForm<VillaFormValues>({
    ...
    defaultValues: villa ? {
      ...
      advancePaymentRate: villa.advancePaymentRate,
      checkInNotes: villa.checkInNotes,
      checkOutNotes: villa.checkOutNotes,
  ...
  async function onSubmit(data: VillaFormValues) {
    try {
      ...
      if (!villa || !villa.id) {
        throw new Error("Villa bilgisi bulunamadı. Lütfen sayfayı yenileyin.");
      }
      ...
      toast.success("Villa başarıyla güncellendi!")
      
      // Başarılı olursa villa listesine yönlendir
      router.push("/admin/villas")
    } catch (error) {
      console.error("Villa güncelleme hatası:", error)
      toast.error("Villa güncellenirken bir hata oluştu")
    } finally {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-end space-x-4">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? "Kaydediliyor..." : villa ? "Villa Güncelle" : "Villa Ekle"}
          </Button>
        </div>
      </form>
    </Form>
