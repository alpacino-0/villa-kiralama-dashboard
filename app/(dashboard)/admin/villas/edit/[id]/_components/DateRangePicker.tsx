...
import type { CalendarEvent, EventType, CalendarStatus, CalendarEventInsert } from "@/types/calendarEvent";
...
export default function DateRangePicker({
  ...
  // Tarih aralığı seçildiğinde Supabase'e kaydet
  const saveCalendarEvents = async () => {
    ...
    try {
      ...
      // Fiyat verilerini tarih bazında eşleştirmek için map oluştur
      const priceMap = new Map();
      if (existingEvents && existingEvents.length > 0) {
        existingEvents.forEach(event => {
          // ISO string veya Date objesini string'e çevirerek saklama
          const dateKey = typeof event.date === 'string' 
            ? event.date.split('T')[0] // ISO string ise T'den önce kısmını al
            : new Date(event.date).toISOString().split('T')[0];
          
          priceMap.set(dateKey, event.price);
        });
      }
      
      // Tüm tarihleri hazırla
      const calendarEntries: CalendarEventInsert[] = [];
      
      // İlk tarih (CHECKIN)
      calendarEntries.push({
        villaId,
        date: startDateISO.toISOString(),
        status: 'AVAILABLE' as CalendarStatus,
        eventType: 'CHECKIN' as EventType,
        price: priceMap.get(startDateISO.toISOString().split('T')[0]) || null
      });
      
      // Aradaki tarihler (BLOCKED)
      let currentDate = addDays(new Date(selectedStartDate), 1);
      while (isBefore(currentDate, selectedEndDate)) {
        const dateISO = new Date(currentDate);
        dateISO.setHours(0, 0, 0, 0);
        const dateKey = dateISO.toISOString().split('T')[0];
        ...
        calendarEntries.push({
          ...
          eventType: null,
          price: priceMap.get(dateKey) || null
        });
      ...
      calendarEntries.push({
        villaId,
        date: endDateISO.toISOString(),
        status: 'AVAILABLE' as CalendarStatus,
        eventType: 'CHECKOUT' as EventType,
        price: priceMap.get(endDateISO.toISOString().split('T')[0]) || null
      });
      
      // Toplu güncelleme yap
      const { error } = await supabase
        .from('CalendarEvent')
        .upsert(calendarEntries, { onConflict: 'villaId,date' })
        .select();
      
      if (error) {
        console.error('Takvim olayları kaydedilirken hata:', error);
        setError('Takvim olayları kaydedilemedi');
        toast.error(`Takvim olayları kaydedilemedi: ${error.message}`);
        return;
      }
      
      // Başarılı mesajı göster
      toast.success(`${calendarEntries.length} tarih başarıyla güncellendi.`);
      
      // Takvim verilerini yeniden yükle
      await fetchCalendarEvents();
      
    } catch (error: unknown) {
      console.error('Takvim olayları kaydedilirken hata oluştu:', error);
      setError('Takvim olayları kaydedilemedi');
      toast.error(`Takvim olayları kaydedilemedi: ${error instanceof Error ? error.message : "Bilinmeyen hata"}`);
    } finally {
      setIsUpdating(false);
    }
  };
      ...
      <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
        ...
        <PopoverContent className="w-auto p-4 max-w-[90vw] sm:max-w-none" align="start">
          <div className="space-y-4">                 
            ...
            {/* Tarih aralığı seçildiğinde kaydetme butonu */}
            {selectedStartDate && selectedEndDate && (
              <Button 
                onClick={saveCalendarEvents}
